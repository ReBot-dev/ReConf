...
<div class={cn("flex flex-col gap-4", className)} {...props}>
  <Switch
    ...
    description={m.tap_hold_on_other_key_press_description()}
    id="hold-on-other-key-press"
    title={m.tap_hold_on_other_key_press()}
  />
  <CommitSlider
    bind:committed={
      () => action.tappingTerm,
    ...
    description={m.tap_hold_tapping_term_description()}
    display={(v) => `${v}ms`}
    min={MIN_TAPPING_TERM}
    max={MAX_TAPPING_TERM}
    step={10}
    title={m.tap_hold_tapping_term()}
  />
  <TickRateSlider />
</div>
