...
<FixedScrollArea class="flex flex-col gap-4 p-4 pt-0">
  <div class="grid text-sm">
    <span class="font-medium">{m.tap_hold_configure_title()}</span>
    <span class="text-muted-foreground">
      {m.tap_hold_configure_description()}
    </span>
  </div>
  <div class="grid place-items-center">
    ...
    >
      <div class="flex flex-col items-center text-center text-base">
        <div class="text-muted-foreground">{m.tap_hold_tap()}</div>
      ...
      <div class="flex flex-col items-center text-center text-base">
        <div class="text-muted-foreground">{m.tap_hold_hold()}</div>
        <div class="p-0.5" style={unitToStyle()}>
          <ToggleGroup.Item
            oncontextmenu={(e) => {
              e.preventDefault()
...
<FixedScrollArea class="flex flex-col gap-4 p-4 pt-0">
  <Tabs.Root value="bindings">
    <Tabs.List>
      <Tabs.Trigger value="bindings">{m.tab_bindings()}</Tabs.Trigger>
      <Tabs.Trigger value="advanced">{m.tab_advanced()}</Tabs.Trigger>
