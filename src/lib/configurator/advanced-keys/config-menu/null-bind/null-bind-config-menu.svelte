...
<script lang="ts">
  ...
  import { socdBehaviorMetadata } from "$lib/configurator/lib/socd"
  import {
...
<FixedScrollArea class="flex flex-col gap-4 p-4 pt-0">
  <div class="grid text-sm text-wrap">
    <span class="font-medium">{m.socd_configure_title()}</span>
    <span class="text-muted-foreground">
      {m.socd_configure_description()}
    </span>
  </div>
  <RadioGroup.Root
    bind:value={
  ...
  >
    {#each socdBehaviorMetadata as { behavior, title, description } (behavior)}
  ...
  <Switch
    ...
    description={m.socd_alt_fully_pressed_description()}
    id="alternative-fully-pressed-behavior"
    title={m.socd_alt_fully_pressed()}
  />
</FixedScrollArea>
<FixedScrollArea class="flex flex-col gap-4 p-4 pt-0">
  <Tabs.Root value="performance">
    <Tabs.List>
      <Tabs.Trigger value="performance">{m.tab_performance()}</Tabs.Trigger>
      <Tabs.Trigger value="key-tester">{m.tab_key_tester()}</Tabs.Trigger>
    </Tabs.List>
    <div class="p-2">
