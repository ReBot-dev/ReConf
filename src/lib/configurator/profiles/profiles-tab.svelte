...
<script lang="ts">
  ...
  const importProfile = async (profile: number) => {
    ...
    fileRef.onchange = async (e) => {
      ...
      try {
        ...
        toast.success(
          m.profiles_tab_import_success({ profile: String(profile) }),
        )
      } catch (err) {
        if (err instanceof SyntaxError) {
          toast.error(m.profiles_tab_import_invalid_json())
        } else if (err instanceof z.ZodError) {
          toast.error(m.profiles_tab_import_invalid_metadata())
  ...
  const exportProfile = async (profile: number) => {
    ...
    try {
      ...
      toast.success(m.profiles_tab_export_success({ profile: String(profile) }))
    } catch (err) {
      if (err instanceof z.ZodError) {
        toast.error(m.profiles_tab_schema_error())
        console.error(z.treeifyError(err))
      } else {
        toast.error(String(err))
      }
...
>
  <FixedScrollArea class="flex flex-col gap-4 p-4">
    <div class="grid shrink-0">
      <span class="font-semibold">{m.profiles_tab_title()}</span>
      <span class="text-sm text-muted-foreground">
        {m.profiles_tab_description()}
      </span>
    </div>
      ...
      {#each { length: numProfiles }, profile (profile)}
        ...
        >
          <div class="grid flex-1 truncate font-semibold">
            {m.profile_select_label({ profile: String(profile) })}
          </div>
          <div class="flex shrink-0 items-center gap-2">
            {#if profile === currentProfile}
              <Badge>{m.profile_active()}</Badge>
            {/if}
            <DropdownMenu.Root>
              <DropdownMenu.Trigger>
                {#snippet child({ props })}
                  <Button size="icon" variant="ghost" {...props}>
                    ...
                    <span class="sr-only">{m.profiles_tab_open_menu()}</span>
                  </Button>
                {/snippet}
              </DropdownMenu.Trigger>
              <DropdownMenu.Content align="start">
                <DropdownMenu.Group>
                  <DropdownMenu.Item onSelect={() => importProfile(profile)}>
                    {m.profiles_tab_import()}
                  </DropdownMenu.Item>
                  <DropdownMenu.Item onSelect={() => exportProfile(profile)}>
                    {m.profiles_tab_export()}
                  </DropdownMenu.Item>
                  <DropdownMenu.Sub>
                    <DropdownMenu.SubTrigger>
                      {m.profiles_tab_duplicate_from()}
                    </DropdownMenu.SubTrigger>
                    <DropdownMenu.SubContent>
                      {#each { length: numProfiles }, srcProfile (srcProfile)}
                        {#if srcProfile !== profile}
                          ...
                          >
                            {m.profile_select_label({
                              profile: String(srcProfile),
                            })}
                          </DropdownMenu.Item>
                        {/if}
                      {/each}
                  ...
                  >
                    {m.profiles_tab_restore_default()}
                  </DropdownMenu.Item>
                </DropdownMenu.Group>
