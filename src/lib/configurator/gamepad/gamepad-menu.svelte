...
>
  <div class="flex items-center gap-4 p-4 pb-0">
    <Tabs.List>
      <Tabs.Trigger value="setup">{m.gamepad_tab_setup()}</Tabs.Trigger>
      <Tabs.Trigger value="analog">{m.gamepad_tab_analog()}</Tabs.Trigger>
    </Tabs.List>
    {#if options && !options.xInputEnabled}
      <Badge variant="destructive">{m.gamepad_xinput_disabled()}</Badge>
    {/if}
  </div>
