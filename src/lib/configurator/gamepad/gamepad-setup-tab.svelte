...
<div class={cn("grid grid-cols-[minmax(0,1fr)_28rem]", className)} {...props}>
  <FixedScrollArea class="flex flex-col gap-4 p-6 pt-2 pr-4">
    <div class="grid text-sm">
      <span class="font-medium">{m.gamepad_configure_title()}</span>
      <span class="text-muted-foreground">
        {m.gamepad_configure_description()}
      </span>
    </div>
  ...
  <FixedScrollArea class="flex flex-col gap-4 p-6 pt-2 pl-4">
    <Switch
      ...
      description={m.gamepad_enable_keyboard_description()}
      disabled={!options}
      id="keyboard-enabled"
      title={m.gamepad_enable_keyboard()}
    />
    <Switch
      bind:checked={
      ...
      description={m.gamepad_override_description()}
      disabled={!options || !options.keyboardEnabled}
      id="gamepad-override"
      title={m.gamepad_override()}
    />
  </FixedScrollArea>
</div>
